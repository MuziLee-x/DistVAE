    ...
    for i, user_id in enumerate(index):
        if is_train:
            if time_split:
                ...
                # cur_train_data = cur_data_x, cur_data_y, cur_padding, user_id, train_cnt - 1
        ...
        value = []
        value.append(train_data_x[i])
        value.append(train_data_y[i])
        value.append(train_padding[i])
        value.append(train_user_id[i])
        value.append(train_cur_cnt[i])
        train_dict[i] = value        

    return torch.utils.data.TensorDataset(train_data_x, train_data_y, train_padding, train_user_id, train_cur_cnt), train_dict


def generate_train_data(hyper_params):
    print('Generating train data...')
    dataset = generate_index_data(hyper_params, index=train_id, is_train=True)
    print('Train data generate succesfully.')
    return dataset


def generate_test_data(hyper_params):
    print('Generating test data...')
    dataset, train_dict = generate_index_data(hyper_params, index=test_id, is_train=False)
