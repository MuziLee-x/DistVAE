...
class FCEncoderAtt(nn.Module):
    def __init__(self, hyper_params, in_shape, out_shape):
        super(FCEncoderAtt, self).__init__()

        self.hyper_params = hyper_params
        self.query = nn.Linear(in_shape, in_shape)
        self.key = nn.Linear(in_shape, in_shape)
        self.value = nn.Linear(in_shape, in_shape)
    ...
    def forward(self, x: torch.Tensor):
        # print(x.shape)   [64,200,100]
        mask = torch.ones([x.shape[1], x.shape[1]], dtype=torch.long)
        for i in range(x.shape[1]):
            mask[i][i+1:] = torch.exp(torch.tensor(-8.0))
            
        if self.hyper_params['add_eps']:
            q = self.query(self.eps(x))
            k = self.key(self.eps(x))
            v = self.value(self.eps(x))
            attention_scores = torch.matmul(q, k.transpose(-1, -2))
            attention_scores = attention_scores / math.sqrt(x.shape[-1])
            attention_mask = torch.mul(attention_scores, mask)
            
            attention_probs = nn.Softmax(dim=-1)(attention_mask)
    ...
    def __init__(self, hyper_params):
        ...
        if hyper_params['model_func'] == 'fc':
            ...
            self.encoder = FCEncoderCNN(hyper_params,
                                        hyper_params['rnn_size'], hyper_params['latent_size'])
        elif hyper_params['model_func'] == 'fc_att':
            self.encoder = FCEncoderAtt(hyper_params,
                                        hyper_params['rnn_size'], hyper_params['latent_size'])
